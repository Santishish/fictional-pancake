...
const EventDetailedHeader = ({event, isHost, isGoing, goingToEvent, cancelGoingToEvent, openModal, authenticated}) => {
    let eventDate;
    if (event.date) {
            ...
            <Segment attached="bottom">
                ...
                <div>
                    {isGoing &&
                        ...
                        {!authenticated &&
                        <Button onClick={() => openModal('UnauthModal')} color="teal">JOIN THIS EVENT</Button>}
                </div>}
                {isHost &&
